...
try:
    ...
    # ── Pure-PyTorch fallback RotaryEmbedding ────────────────────────
    class RotaryEmbedding(nn.Module):  # type: ignore[no-redef]
        """Minimal RotaryEmbedding compatible with flash_attn API.

        Supports the subset of the API used by MHA:
          - __init__(dim, base, interleaved, device)
          - __call__(q, kv, seqlen_offset, max_seqlen)
          - _update_cos_sin_cache(seqlen, device, dtype)
          - _cos_cached / _sin_cached attributes
        ...
        def _update_cos_sin_cache(self, seqlen: int, device=None, dtype=None):
            ...
            # [seqlen, dim] — duplicate for paired dims
        ...
        def _apply_rotary(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
            """Apply rotary to the first self.dim dims of x [..., head_dim]."""
            rd = self.dim
            x_rot = x[..., :rd]
            x_pass = x[..., rd:]
        ...
        def forward(
            ...
            if isinstance(seqlen_offset, int):
                total_len = seqlen + seqlen_offset
            else:
                total_len = seqlen + int(seqlen_offset.max().item())
            if max_seqlen is not None:
                total_len = max(total_len, max_seqlen)
            self._update_cos_sin_cache(total_len, device=q.device, dtype=q.dtype)
            assert self._cos_cached is not None and self._sin_cached is not None

            if isinstance(seqlen_offset, int):
                cos = self._cos_cached[seqlen_offset : seqlen_offset + seqlen].unsqueeze(0).unsqueeze(2)
                sin = self._sin_cached[seqlen_offset : seqlen_offset + seqlen].unsqueeze(0).unsqueeze(2)
            else:
                # per-sample offsets — rare in training, common in generation
                cos = self._cos_cached[:seqlen].unsqueeze(0).unsqueeze(2)
                sin = self._sin_cached[:seqlen].unsqueeze(0).unsqueeze(2)
