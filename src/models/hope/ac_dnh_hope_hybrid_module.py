"""Lightning Module for AC-DNH-HOPE-Hybrid-ViT architecture (Phase 11).

Wraps ACDNHHOPEHybridViT with the same training / evaluation pipeline as
ACHOPEHybridModule (Phase 8):
...
ADDITIONS for DNH (Phase 11):
    - StructuralEvolutionController per block (add/prune/frequency modulation)
    - Meta-loss computation for structural evolution decisions
    - Per-block evolution tracking and logging
    - 5-group optimizer (attention, dnh_memory, dnh_controller, cms, projections)

Data contract (identical to Phase 8):
    Input batch:
...
from src.models.hope.ac_dnh_hope_hybrid_vit import ac_dnh_hope_hybrid_vit
...
class ACDNHHOPEHybridModule(TTAMixin, ACPredictorLossMixin, L.LightningModule):
    """PyTorch Lightning module for AC-DNH-HOPE-Hybrid-ViT predictor (Phase 11).

    Structurally parallel to ACHOPEHybridModule (Phase 8) but uses DNH components:
        - DynamicNestedHierarchy per block (Phase B)
        - DynamicCMS per block (Phase C)
        - StructuralEvolutionController per block
    ...
    def __init__(
        ...
        # DNH structural evolution
        dnh_tau_add: float = 0.5,
        dnh_epsilon_prune: float = 0.01,
        dnh_gamma_freq: float = 0.1,
        dnh_eta_freq: float = 0.01,
        dnh_beta_momentum: float = 0.9,
        dnh_evolution_interval: int = 50,
        dnh_evolution_warmup: int = 200,
        dnh_enable_addition: bool = True,
        dnh_enable_pruning: bool = True,
        dnh_enable_freq_modulation: bool = True,
        # Meta-loss weights
        dnh_meta_lambda: float = 0.01,
        dnh_meta_mu: float = 0.001,
        # Optimizer: per-group LR/WD scaling
        attention_lr_scale: float = 1.0,
        dnh_memory_lr_scale: float = 0.1,
        dnh_controller_lr_scale: float = 0.05,
        cms_lr_scale: float = 0.6,
        projections_lr_scale: float = 1.0,
        titan_weight_decay: float | None = None,
        dnh_controller_weight_decay: float = 0.0,
        ...
        **kwargs: Any,
        ...
        self.model = ac_dnh_hope_hybrid_vit(
            img_size=img_size,
        ...
        # ─── Structural evolution controllers (one per block) ───
        evolution_cfg = EvolutionConfig(
            tau_add=dnh_tau_add,
            epsilon_prune=dnh_epsilon_prune,
            gamma_freq=dnh_gamma_freq,
            eta_freq=dnh_eta_freq,
            beta_momentum=dnh_beta_momentum,
            evolution_interval=dnh_evolution_interval,
            enable_addition=dnh_enable_addition,
            enable_pruning=dnh_enable_pruning,
            enable_freq_modulation=dnh_enable_freq_modulation,
            warmup_steps=dnh_evolution_warmup,
        )
        self.evolution_controllers = [
            StructuralEvolutionController(evolution_cfg, max_levels=dnh_L_max)
            for _ in range(depth)
        ]
        # Initialize tracking
        for ctrl in self.evolution_controllers:
            ctrl.initialize_tracking(dnh_L_init)

        # Meta-loss weights
        self.dnh_meta_lambda = dnh_meta_lambda
        self.dnh_meta_mu = dnh_meta_mu
        ...
        self.dnh_memory_lr_scale = dnh_memory_lr_scale
        self.dnh_controller_lr_scale = dnh_controller_lr_scale
        self.cms_lr_scale = cms_lr_scale
        self.projections_lr_scale = projections_lr_scale
        self.titan_weight_decay = titan_weight_decay
        self.dnh_controller_weight_decay = dnh_controller_weight_decay
        ...
        # Previous hidden state for meta-loss drift penalty
        self._previous_hidden: Tensor | None = None
    ...
    def _validate_temporal_dimensions(self, T_teacher: int, jump_k: int) -> None:
        ...
        if T_teacher > max_steps:
            warnings.warn(
                f"T_teacher ({T_teacher}) exceeds available steps ({max_steps}).",
    ...
    def forward(
        ...
        """Forward pass through the DNH Hybrid predictor."""
    ...
    def training_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        """Training step with memory reset, structural evolution, and diagnostics."""
        self.model.reset_all_memories()

        # Compute task loss
        loss = self._shared_step(batch, "train")

        # ─── Structural evolution (DNH-specific) ───
        meta_loss_val = loss.detach().item()

        # Execute evolution step on all blocks
        evolution_events = self.model.step_structural_evolution(
            self.evolution_controllers, meta_loss_val
        )

        # Log evolution events
        if batch_idx % self._diagnostics_log_interval == 0:
            for key, val in evolution_events.items():
                self.log(f"train/{key}", val, sync_dist=True)

            # Log structural summary
            summary = self.model.get_structural_summary()
            avg_mem_levels = sum(summary["num_memory_levels"]) / len(summary["num_memory_levels"])
            avg_cms_levels = sum(summary["num_cms_levels"]) / len(summary["num_cms_levels"])
            self.log("train/dnh/avg_memory_levels", avg_mem_levels, sync_dist=True)
            self.log("train/dnh/avg_cms_levels", avg_cms_levels, sync_dist=True)
    ...
    def on_fit_start(self) -> None:
        ...
        log.info(f"[AC-DNH-HOPE-Hybrid-ViT] Configuration: {config}")
        if self.logger:
    ...
    def on_test_epoch_end(self) -> None:
        ...
        # Log structural summary at test time
        summary = self.model.get_structural_summary()
        avg_mem = sum(summary["num_memory_levels"]) / len(summary["num_memory_levels"])
        avg_cms = sum(summary["num_cms_levels"]) / len(summary["num_cms_levels"])

        print(f"\n{'='*70}")
        print("       AC-DNH-HOPE-HYBRID-ViT TEST RESULTS SUMMARY")
        print(f"{'='*70}")
        print(f"\n  Aggregate (over {num_clips} clips)")
        print(f"    Mean:   {mean_loss:.6f}")
        print(f"    Median: {median_loss:.6f}")
        print(f"    Avg memory levels: {avg_mem:.1f}")
        print(f"    Avg CMS levels: {avg_cms:.1f}")
        ...
        try:
            ...
            results_file.write_text(json.dumps({
                "model_type": "AC-DNH-HOPE-Hybrid-ViT",
                "summary": {
                    "num_clips": num_clips,
                    "jump_loss": {"mean": mean_loss, "median": median_loss},
                    "per_timestep": per_timestep_stats,
                    "structural": {
                        "avg_memory_levels": avg_mem,
                        "avg_cms_levels": avg_cms,
                        "memory_levels_per_block": summary["num_memory_levels"],
                        "cms_levels_per_block": summary["num_cms_levels"],
                    },
                },
    ...
    def configure_optimizers(self) -> dict[str, Any]:
        """Configure AdamW optimizer with 5 per-group learning rates."""
        ...
        for group in param_groups:
            ...
            elif name == "dnh_memory":
                lr = self.learning_rate * self.dnh_memory_lr_scale
                wd = self.titan_weight_decay if self.titan_weight_decay is not None else self.weight_decay
            elif name == "dnh_controller":
                lr = self.learning_rate * self.dnh_controller_lr_scale
                wd = self.dnh_controller_weight_decay
            elif name == "cms":
