Based on your clarification regarding your training strategy (intentionally shifting weight from Teacher Forcing to Rolling Loss to strengthen autoregressive performance) and the provided context, here is the revised recommendation in English.
...
Take the action vector for time $t$.
Replicate (tile) it across the spatial dimensions ($16 \times 16$).
Concatenate it with the features from Module A.
...
Would you like me to write out the full PyTorch code for the ConvLSTMCell and the SpatialTiling module?
